...
`<os-temporary-directory>/antigravity`. Each workspace leaf is named from a
sanitized basename plus a 12-character hash of the resolved (realpath) path.
A leaf written under the older logical spelling stays in use until the
realpath leaf exists. A background worker receives the caller's exact
workspace spelling. Codex and agy installations upgraded from a version that
ignored their host variable continue to read and write an existing workspace
state directory at that legacy temporary location. New workspaces use their
host-owned data directory.
...
| `vision` receives `--add-dir`.<br><br>Command: `node bin/antigravity.mjs vision pixel.png --add-dir C:\scope`. | `antigravity:vision — unknown flag --add-dir; put prompt text after --` Exit 1. | Remove `--add-dir`. Name each image path on the command line. |
