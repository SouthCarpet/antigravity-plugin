...
![A host command from Claude Code, Codex CLI, the agy TUI, or a plain shell — the eight verbs setup, review, rescue, task, vision, status, result, and cancel — enters the plugin runtime at bin/antigravity.mjs and scripts/. The runtime talks to agy --print over stream-json; agy talks to Google. Only the prompt, the selected diff, and named image bytes leave this machine; nothing else does. For vision, the runtime starts a local MCP server that exposes one allowlisted tool, view_image, and agy calls back into it. A background job's request, result, and log stay in the local job store on this machine; status, result, and cancel read that store and never reach Google. When agy refuses a tool in headless mode, the plugin reports the refused action and, since 1.4.0, the target it was refused on, tells the host to ask the user, and never grants the tool or prints a bypass flag.](./docs/how-it-works.svg)
